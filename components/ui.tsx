  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-6">
      ...
      <div
        ...
        className={clsx(
          "relative w-full rounded-card border border-line bg-white shadow-[0_24px_70px_rgba(27,31,42,0.22)]",
          sizes[size]
        )}
      >
...
export function Field({
  ...
  return (
    <label className={clsx("block", className)}>
      <span className="text-[11px] text-ink-muted block mb-1">{label}</span>
      {children}
    </label>
  );
}
  ...
  return (
    <div className="flex items-start justify-between gap-4 mb-4">
      <div>
        ...
        {description && <p className="text-sm text-ink-muted mt-1 max-w-xl">{description}</p>}
      </div>
...
export function Avatar({ initials, size = 36 }: { initials: string; size?: number }) {
  return (
    <div
      className="rounded-full bg-brand-500 text-white flex items-center justify-center font-medium shrink-0"
