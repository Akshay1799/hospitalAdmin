...
import { InsuranceNav } from "@/components/insurance-tpa/InsuranceNav";
import { mockInsurancePatients } from "@/lib/mock-data/insurance-tpa-extended";
import { InsurancePatientRecord } from "@/lib/types";
    ...
    return (
      <div className="space-y-4 animate-fade-in pb-12">
        <PageHeader
          ...
          crumbs={[{ label: "Finance" }, { label: "Insurance / TPA", href: "/insurance-tpa" }, { label: "Patients" }]}
        />
              ...
              <TableBody>
                {filtered.map((p) => (
                  <TableRow key={p.id} className="hover:bg-muted/30 text-xs">
                    <TableCell>
                      <Link href={`/patients`} className="font-semibold text-foreground hover:text-primary hover:underline">
                        {p.patientName}
                      </Link>
