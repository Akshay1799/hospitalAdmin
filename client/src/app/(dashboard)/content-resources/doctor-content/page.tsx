...
import { ContentResourcesNav } from "@/components/content-resources/content-resources-nav";
import { AuthorDoctorContentModal } from "@/components/content-resources/AuthorDoctorContentModal";
import { ClinicalSignOffModal } from "@/components/content-resources/ClinicalSignOffModal";
import {
  addDoctorContent,
  approveClinicalReview,
  rejectClinicalReview,
  publishToPublicPortal,
} from "@/store/slices/contentResourcesSlice";
import { DoctorContentItem, ClinicalReviewRecord } from "@/lib/types";

export default function DoctorContentPage() {
  const dispatch = useDispatch<AppDispatch>();
  const doctorContent = useSelector((s: RootState) => s.contentResources.doctorContent);
  ...
  const filteredContent = doctorContent.filter((item) => {
    const matchesSearch =
  ...
  const handleOpenReview = (item: DoctorContentItem) => {
    setReviewModalState({
      isOpen: true,
      contentId: item.id,
  ...
  const handlePublish = (itemId: string) => {
    dispatch(publishToPublicPortal({ id: itemId, category: "Doctor Content" }));
  };
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-bold tracking-tight text-foreground">
                Doctor-Attributed Clinical Content
              </h1>
            </div>
            <p className="mt-1 text-xs text-muted-foreground">
              Clinical pearls, surgical case studies, and research guides linked directly to verified physician records and feeding Hospital Profile (F24).
            </p>
          </div>
      ...
      <div className="flex-1 space-y-6 p-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-1">
            ...
            >
              All ({doctorContent.length})
            </Button>
            <Button
              size="sm"
              variant={typeFilter === "health tip" ? "default" : "outline"}
              onClick={() => setTypeFilter("health tip")}
              className="h-8 text-xs"
            >
              Health Tips
            </Button>
            <Button
              size="sm"
              variant={typeFilter === "case study" ? "default" : "outline"}
            ...
            <Button
              size="sm"
              variant={typeFilter === "specialty guide" ? "default" : "outline"}
              onClick={() => setTypeFilter("specialty guide")}
              className="h-8 text-xs"
            >
              Specialty Guides
            </Button>
                ...
                <div className="rounded-lg border border-border bg-muted/30 p-2.5 flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2">
                    <div className="flex h-7 w-7 items-center justify-center rounded-full bg-primary/10 text-primary font-bold text-[11px]">
                      {item.doctorName.split(" ")[1]?.charAt(0) || "D"}
                    </div>
                    <div>
                      <div className="flex items-center gap-1">
                        <span className="font-semibold text-foreground">{item.doctorName}</span>
                        {item.isDoctorVerified && (
                          <ShieldCheck className="h-3.5 w-3.5 text-emerald-600" />
                        )}
                      </div>
                      <span className="text-[10px] text-muted-foreground">{item.doctorSpecialty}</span>
                    </div>
                  </div>
                  {item.doctorUhid && (
